...
class Agent:
    ...
    Includes use of:
      - State of neighboring cells (option)
    ...
    def get_neighbors_state(self, grid):
        """
        Returns an 8-bit string encoding neighboring occupancy (N, NE, E, SE, S, SW, W, NW)
        Used for the 'state of neighboring cells' sensor input.
        """
    ...
    def get_state(self, grid):
        neighbors = self.get_neighbors_state(grid)
...
for episode in range(200):
    ...
    for step in range(20):
        for agent in agents:
            state = agent.get_state(grid)
            action = agent.choose_action(grid)
            old_pos = agent.pos
            agent.move(action)

            reward = 5 if (old_pos == A_LOCATION and agent.pos == B_LOCATION) else 0
            for other in agents:
                if other is not agent and agent.pos == other.pos:
                    if agent.carrying != other.carrying:
                        reward = -10

            next_state = agent.get_state(grid)
